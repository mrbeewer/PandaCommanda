...
public class BlockSpawner : NetworkBehaviour {
	...
	public GameObject CmdSpawnBlockRandomColor(float speed){
	...
	public void SpawnBlockAtSpawnPoint(int indexOfSpawnPoint, float speed){
		CmdSpawnBlockRandomColor (speed).transform.position = spawnPoints [indexOfSpawnPoint].position;
	}
	...
	IEnumerator SpawnSeperationTimer(int time, float speed){
		...
		CmdSpawnBlockRandomColor (speed);
